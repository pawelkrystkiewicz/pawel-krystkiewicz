...
export const AchievementEntry: FC<AchievementEntryProps> = ({
  achievement,
}) => {
  return (
    <div key={achievement.company} className='flex flex-col gap-1'>
      <h5 className='font-bold'>
        {achievement.project && (
          <span className='text-text-secondary'>{achievement.project} @</span>
        )}
        {achievement.company}
      </h5>
      {achievement.entries.map(entry => (
        <div
          ...
          className={clsx(
            'grid gap-1',
            entry.source ? 'grid-rows-2 print:grid-rows-1' : 'grid-rows-1',
          )}
        >
          <p
